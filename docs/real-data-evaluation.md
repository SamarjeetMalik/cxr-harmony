...
|---|---|---|
| [Open-i / Indiana University](https://openi.nlm.nih.gov/faq) | 3,955 real radiology reports with manually assigned MeSH terms; 3,927 carry usable sectioned text | CC BY-NC-ND 4.0 |
...
> Plotted, with the full target-versus-achieved comparison, in
...
| Real, before fixes | 0.844 | 0.800 | 0.822 | 0.731 | 0.900 |
...
than fitting the tuning half. Cohen's κ on the held-out half is **0.897** against
the project's >0.80 target; the "before fixes" row scored κ 0.814, which passes the
same target on much worse F1 — a reminder that κ alone is not sufficient.

Every figure and number here is regenerated by `make results`; the "before" row
comes from `scripts/evaluate_openi.py --baseline`, not from notes.
...
accounted for most false-positive pneumothorax calls — precision 0.286 before,
0.800 after, F1 0.436 to 0.857.
...
was 0.333 before those were added, 0.860 after.
...
to 0.600 with recall 0.306 on the held-out half — and, on the full corpus, precision
0.306 with 220 false positives. Radiologist annotators index those under a separate
...
with *"No acute cardiopulmonary disease"*. Normal-study F1 rose from 0.452 to 0.674.
